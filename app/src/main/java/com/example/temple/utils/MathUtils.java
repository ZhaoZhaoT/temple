...
public class MathUtils {
    ...
    public static String stripTrailingZeros(double progressValue, int decimal) {
        if (progressValue == 0) {
            return "0";
        }
        return new BigDecimal(Float.toString((float) progressValue))
                .setScale(decimal, RoundingMode.HALF_UP).stripTrailingZeros().toPlainString();
    }
    ...
    //不保留小数 不四舍五入
    public static String roundingMode(double value) {
//        return String.valueOf((int)Math.floor(value));
    ...
    //不保留小数 四舍五入
    public static String roundingModeTwo(double value) {
//        return String.valueOf((int)Math.floor(value));
        DecimalFormat myformat = new DecimalFormat("0");
    ...
    //保留两位位小数
    public static String roundingMoreTwoMode(double value) {
    ...
    public static double large(double a, double b, double c, double d) {
        double r = a;
        if (r < b) r = b;
        if (r < c) r = c;
        if (r < d) r = d;
